...
zScore<-function(bpm,Group) {
  Gr<-get()
} (bpm-mean(Group))/sd(Group)

#data to calculate zScore for 1 athlete and 1 control
mBPM<-round(meanA-rnorm(1,2.5,.2)*sdA)
cBPM<-round(meanC-rnorm(1,1.5,.2)*sdC
...
- Explain your answer.
- A nurse practitioner examines two subjects from the above study.  The first, a marathon runner, has resting heart rate of `r mBPM`. The second is from the control group and has a resting heart rate of `r cBPM)`.
    - Calculate the Z scores for both subjects:  Marathoner Z=            Control Z=   
    - Which one shows the most extreme difference in heart rate compared to other members of their group (athletes vs. control)?
...
- Using the 68-95-99.7 rule what is the range of resting heart rate values you would expect for the central 68% of the control group.  Lower range=      ?   Upper range=       ?
- What percent of the control group will have a heart rate between `r round(meanC-(.4*sdC))`bpm and `r round(meanC+(.6*sdC))`bpm

- What percent of the marathon group will have a heart rate between `r round(meanA-(.6*sdA))`bpm and `r round(meanA+(.4*sdA))`bpm
