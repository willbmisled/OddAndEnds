...
* All for distributions are more or less normal; there is slight right skew to the TSI_P
...
* This is the confusion matrix for comparing Trophic State Assignment by Chlorophyll and Secchi

```{r confuseS, include=TRUE, echo=FALSE, warning=FALSE}
confusionMatrix(table(Chlorophyll=tsi$ts_c,Secchi=tsi$ts_s))

```

* we can also compare Trophic State Assignment by Chlorophyll and Secchi graphically

```{r graphS, include=TRUE, echo=FALSE, warning=FALSE}
#plot by whether ts_n and ts_s agree

ts_dif<-factor(abs(as.numeric(tsi$ts_s)-as.numeric(tsi$ts_c)))  #how much do the TS categories differ 0=no dif.

ggplot(data=tsi)+
    geom_point(mapping=aes(x=tsi_s,y=tsi_c,color=ts_dif))+
